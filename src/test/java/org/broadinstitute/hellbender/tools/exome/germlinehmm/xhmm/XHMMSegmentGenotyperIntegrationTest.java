...
public class XHMMSegmentGenotyperIntegrationTest extends XHMMSegmentCallerBaseIntegrationTest {
    ...
    private void assertVariantSegmentsAreCovered(final List<VariantContext> variants,
        ...
        for (final HiddenStateSegmentRecord<CopyNumberTriState, Target> variantSegment : variantSegments) {
            ...
            final double expectedCallPL = GATKProtectedMathUtils.roundPhred(QualityUtils.phredScaleErrorRate(QualityUtils.qualToProb(variantSegment.getSegment().getExactQuality())), HiddenMarkovModelPostProcessor.PHRED_SCORE_PRECISION);
            final double expectedRefPL = GATKProtectedMathUtils.roundPhred(QualityUtils.phredScaleCorrectRate(QualityUtils.qualToProb(variantSegment.getSegment().getEventQuality())), HiddenMarkovModelPostProcessor.PHRED_SCORE_PRECISION);
